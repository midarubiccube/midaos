...
bool IsConnectUSBkeyboard() {
  auto mgr = usb::xhci::controller->DeviceManager();
  for (auto i = 0; i < mgr->GetMaxSlot(); ++i) {
    auto entry = mgr->FindBySlot(i);
    if(entry) return entry->CheckClassDriver();
    Log(kError, "%d\n", i);
  }
  return false;
}
