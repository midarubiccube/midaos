...
namespace {
  ...
  void SwitchEhci2Xhci(pci::PCIDevice& xhc_dev) {
    auto intel_ehc = pci::FindDevice(0x0cu, 0x03u, 0x20u);
  
    if (intel_ehc) {
      return;
    }

    uint32_t superspeed_ports = xhc_dev.ReadConfReg(0xdc); // USB3PRM
    xhc_dev.WriteConfReg(0xd8, superspeed_ports); // USB3_PSSEN
    uint32_t ehci2xhci_ports = xhc_dev.ReadConfReg(0xd4); // XUSB2PRM
    xhc_dev.WriteConfReg(0xd0, ehci2xhci_ports); // XUSB2PR
...
namespace usb::xhci {
  ...
  void Initialize() {
    ...
    pci::PCIDevice* xhc_dev = pci::FindDevice(0x0cu, 0x03u, 0x30u);
    if (xhc_dev) {
    ...
    const WithError<uint64_t> xhc_bar = xhc_dev->ReadBar(0);
