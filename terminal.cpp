...
void Terminal::ExecuteLine() {
  ...
  } else if (strcmp(command, "lsdisk") == 0) {
    char s[256];
    for (auto ata : ata::serial_ata_connect_ports) {
      auto info = ata->GetInfo();
      sprintf(s, "AHCI Connect Port %d\nModel_number: %.40s\nSerial_Number: %.20s\nFirmware_revision: %.8s\n",
        ata->GetPortnum(), info->bits.Model_number, info->bits.Serial_Number, info->bits.Firmware_revision);
      Print(s);
    }
  } else if (strcmp(command, "infoahci") == 0) {
    char s[256];
    sprintf(s, "Command slot nun: %d\n", ata::ghc->cap.bits.NCS + 1);
    Print(s);
    sprintf(s, "64-bit Addressing: %s\n", ata::ghc->cap.bits.S64A ? "true" : "false");
    Print(s);
    sprintf(s, "Supports Staggered Spin-up: %s\n", ata::ghc->cap.bits.SSS ? "true" : "false");
    Print(s);
    sprintf(s, "Supports AHCI mode only: %s\n", ata::ghc->cap.bits.SPM ? "true" : "false");
    Print(s);
    sprintf(s, "Supports Port Multiplier: %s\n", ata::ghc->cap.bits.SSS ? "true" : "false");
    Print(s);
    sprintf(s, "Supports Activity LED: %s\n", ata::ghc->cap.bits.SAL ? "true" : "false");
    Print(s);
