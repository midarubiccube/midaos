...
extern "C" void KernelMainNewStack(
  ...
  if (pci::FindDevice(0x0cu, 0x03u, 0x30u)){
    usb::xhci::Initialize();
    InitializeKeyboard();
    InitializeMouse();
  }
  //printk("IsConnect USBKeyboard %s", IsConnectUSBkeyboard() ? "true" : "false");
    ...
    case Message::kTimerTimeout:
      if (msg->arg.timer.value == kTextboxCursorTimer) {
        ...
        timer_manager->AddTimer(
            Timer{msg->arg.timer.timeout + kTimer05Sec, main_task.ID(), kTextboxCursorTimer});
        __asm__("sti");
    ...
    case Message::kKeyPush:
      if (auto act = active_layer->GetActive(); act == text_window_layer_id) {
        ...
        if (task_it != layer_task_map->end()) {
          ...
          printk("key push not handled: keycode %02x, ascii %s\n",
